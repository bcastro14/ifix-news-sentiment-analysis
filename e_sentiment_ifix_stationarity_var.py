...
df_var_title = df[['ifix_value', 'title_sentiment']]
model_title = VAR(df_var_title)

lag_order = model_title.select_order(maxlags=30)
print(lag_order.summary())

#%%
# Get the lag corresponding to the lowest BIC
optimal_lag_bic = lag_order.bic
var_model_title_bic = model_title.fit(optimal_lag_bic)
print(var_model_title_bic.summary())

#%%
# Get the lag corresponding to the lowest AIC
optimal_lag_aic = lag_order.aic
var_model_title_aic = model_title.fit(optimal_lag_aic)
print(var_model_title_aic.summary())

#%%
########## Testing the result of the model with snippet_sentiment ##########
df_var_snippet = df[['ifix_value', 'snippet_sentiment']]
model_snippet = VAR(df_var_snippet)

lag_order = model_snippet.select_order(maxlags=30)
print(lag_order.summary())

#%%
# Get the lag corresponding to the lowest BIC
optimal_lag_bic = lag_order.bic
var_model_snippet_bic = model_snippet.fit(optimal_lag_bic)
print(var_model_snippet_bic.summary())

#%%
# Get the lag corresponding to the lowest AIC
optimal_lag_aic = lag_order.aic
var_model_snippet_aic = model_snippet.fit(optimal_lag_aic)
print(var_model_snippet_aic.summary())